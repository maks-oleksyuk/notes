...
describe('HttpClient', () => {
  ...
  it('wraps a raw fetch failure (not abort/timeout) as NetworkError', async () => {
    fetchMock.mockRejectedValue(
      new TypeError('fetch failed: get addr info ENOTFOUND'),
    );
    const client = new HttpClient('https://api.test');
  ...
  it('falls back to "json" when responseType is explicitly undefined', async () => {
    ...
    // A key present with an `undefined` value overrides the constructor's default ('json')
    // during the option spread — the `|| 'json'` fallback in `attempt()` exists specifically for this case.
  ...
  describe('timeout', () => {
    it('aborts a hanging request after `timeout` and throws a retryable TimeoutError', async () => {
      // Real `fetch` rejects with the signal's own abort *reason* (spec behavior) —
      // `AbortSignal.timeout()`'s reason is a DOMException already named
  ...
  describe('recovery phase (onError)', () => {
    ...
    it('replays via context.retry() with the mutated mergedOptions (not the original options)', async () => {
      ...
      // survived into the new request — otherwise the auth plugin's loop guard
      // couldn't tell a replay from a first attempt.
  ...
  describe('GET dedup (CP-8, opt-in)', () => {
    ...
    it('does not dedupe when a Cookie header carries identity (B5, review 07-07)', async () => {
      fetchMock.mockImplementation(async () => jsonResponse({ ok: true }));
      const client = new HttpClient('https://api.test', {
    ...
    it("does not dedupe with credentials: 'include' — cookies ride along implicitly (B5, review 07-07)", async () => {
      fetchMock.mockImplementation(async () => jsonResponse({ ok: true }));
      const client = new HttpClient('https://api.test', {
        credentials: 'include',
      });
