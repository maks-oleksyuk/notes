...
import { z } from 'zod';
import { HttpClient } from '@/lib/http-client/core';
import { validation } from '@/lib/http-client/plugins';
...
```ts
import { HttpClient } from '@/lib/http-client/core';
import { auth, type TokenProvider } from '@/lib/http-client/plugins';
...
(і тому не з `@/lib/http-client/plugins`) — він імпортує `next/headers`, який ламає збірку,
якщо потрапить у клієнтський бандл через баррель, яким користуються браузерні
клієнтські компоненти. Імпортуй напряму:
`import { createServerTokenProvider } from '@/lib/http-client/plugins/auth/server-provider'`.
