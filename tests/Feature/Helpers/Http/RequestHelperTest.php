...
it('converts falsy values to boolean false', function () {
...
it('returns null for invalid boolean values', function () {
    expect(RequestHelper::toBoolean('invalid'))->toBeNull();
});

it('parses a comma-separated string into an array of strings', function () {
    $input = 'apple, banana, orange';
    $expected = ['apple', 'banana', 'orange'];
    expect(RequestHelper::parseStringIntoArray($input))->toBe($expected);
});

it('trims items and filters out empty strings', function () {
    $input = '  apple  , , banana,   orange,';
    $expected = ['apple', 'banana', 'orange'];
    expect(RequestHelper::parseStringIntoArray($input))->toBe($expected);
});

it('parses a comma-separated string into an array of integers', function () {
    $input = '1,2,3';
    $expected = [1, 2, 3];
    expect(RequestHelper::parseStringIntoIntArray($input))->toBe($expected);
});

it('filters out non-numeric values when parsing into int array', function () {
